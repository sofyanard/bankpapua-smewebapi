...
{
    [Authorize]
    [RoutePrefix("api/NotarisUpload")]
    public class NotarisUploadController : ApiController
    {
        private LOSSME db = new LOSSME();

        // GET: api/NotarisUpload
        [Route("{apRegno}")]
        public IHttpActionResult GetAll(string apRegno)
        {
            List<DocUploadFileUpload> listDocUploadFileUpload = db.DocUploadFileUploads
                .Where(p => p.ApRegno == apRegno && p.GroupFile == "NTRUPLOAD")
                .OrderBy(p => p.Seq)
                .ToList();
        ...
        public async Task<HttpResponseMessage> PostFormData(string apRegno)
        {
            ...
            try
            {
                ...
                docUploadFileUpload.GroupFile = "NTRUPLOAD";
